#!/usr/bin/env python3
"""
Script MEJORADO v3.0 para limpiar COMPLETAMENTE el dataset de traducciones malformado
"""
...
def clean_full_dataset_v3():
    print("🧹 Limpiando dataset COMPLETO v3.0...")
    
    input_path = Path('datasets/muestra_traducciones_10000.csv')
    output_path = Path('datasets/clean_full_traducciones_v3.csv')
    ...
    for i, line in enumerate(lines):
        ...
        # Patrón 1: "número,""catalán"",""chino"" (comillas escapadas con ;;;)
        match_quoted_with_semicolons = re.match(r'^"(\d+),""(.+)"",""(.+)""$', line_clean)
        if match_quoted_with_semicolons:
            line_num, catalan, chino = match_quoted_with_semicolons.groups()
        ...
        # Patrón 2: "número,""""""catalán"""""","""chino"""" (comillas múltiples)
        match_quoted_multiple = re.match(r'^"(\d+),""""""(.+)"""""","""(.+)""""$', line_clean)
        ...
        # Patrón 3: número,catalán,chino (separado por comas simple)
        parts = line_clean.split(',', 2)  # Dividir solo en 3 partes
        if len(parts) == 3:
        ...
        # Patrón 4: "número,"catalán","chino"" (comillas normales)
        match_normal_quotes = re.match(r'^"(\d+),"(.+)","(.+)""$', line_clean)
        ...
        # Patrón 5: "número,""catalán"",""chino"";;; (con ;;; al final)
        match_quoted_semicolons = re.match(r'^"(\d+),""(.+)"",""(.+)"";;;$', line)
        if match_quoted_semicolons:
            line_num, catalan, chino = match_quoted_semicolons.groups()
            catalan = catalan.replace('""', '"')
            chino = chino.replace('""', '"')
            parsed_data.append({
                'catalan': catalan,
        ...
        # Si no coincide con ningún patrón, mostrar advertencia solo para las primeras 10
        if i < 10:
    ...
    if not df.empty:
        ...
        print(f"🎉 ¡Dataset limpio v3.0 guardado!")
