...
def merge_lora_weights(
    base_weight: torch.Tensor, lora_A: torch.Tensor, lora_B: torch.Tensor, scaling: float = 1.0
) -> torch.Tensor:
    ...
    # Verify dimensiones compatibles
    if lora_A.dim() != 2 or lora_B.dim() != 2:
        raise ValueError("lora_A y lora_B deben ser matrices 2D")

    # Calculate el peso LoRA: B @ A
    lora_weight = torch.matmul(lora_B, lora_A) * scaling

    # Verify que las dimensiones coincidan con el peso base
    if lora_weight.shape != base_weight.shape:
        raise ValueError(
            f"Dimensiones incompatibles: lora_weight {lora_weight.shape} vs base_weight {base_weight.shape}"
        )
...
def merge_lora_adapter(
    lora_model_path: str, output_path: str, push_to_hub: bool = False, hub_model_id: str = None
):
    ...
    if not (lora_path / "adapter_config.json").exists():
        console.print(f"[red]Error: Not found adapter_config.json en {lora_path}[/red]")
        sys.exit(1)

    # Create directorio de salida
    output_path.mkdir(parents=True, exist_ok=True)

    console.print("[bold cyan] Fusionando modelo LoRA[/bold cyan]")
    ...
    try:
        ...
        if not Path(base_model_path).exists():
            ...
            if not base_model_path.exists():
                console.print(f"[red]Error: Not found el modelo base en: {base_model_path}[/red]")
                console.print("\nOpciones:")
        ...
        with Progress(
            ...
            # Save modelo fusionado
            task5 = progress.add_task("Saving modelo fusionado...", total=1)
        ...
        console.print(f"Size del modelo: {total_size / (1024**3):.2f} GB")
        ...
        if push_to_hub and hub_model_id:
            ...
            try:
                ...
                # Create repositorio si no existe
...
def verify_merged_model(model_path: Path):
    ...
    try:
        ...
        # Load modelo (solo config para verificación rápida)
        ...
        console.print(f"   Layers: {getattr(config, 'num_hidden_layers', 'N/A')}")

        # Verify archivos del modelo
...
Ejemplos:
  ...
  python merge_lora.py ./mi_lora ./output --push-to-hub --hub-model-id usuario/modelo
    ...
    parser.add_argument(
        "lora_model", type=str, help="Ruta al modelo LoRA (con adapter_config.json)"
    )

    parser.add_argument("output_path", type=str, help="Ruta de salida para el modelo fusionado")

    parser.add_argument(
        "--base-model", type=str, help="Ruta al modelo base (si no se detecta automáticamente)"
    )

    parser.add_argument(
        "--push-to-hub", action="store_true", help="Subir el modelo fusionado a HuggingFace Hub"
    )

    parser.add_argument(
        "--hub-model-id", type=str, help="ID del modelo en HuggingFace Hub (usuario/modelo)"
    )

    parser.add_argument(
        "--no-verify", action="store_true", help="Omitir verificación del modelo fusionado"
    )

    parser.add_argument(
        "--force", action="store_true", help="Sobrescribir si el directorio de salida existe"
    )
    ...
    if not lora_path.exists():
        console.print(f"[red]Error: Not found el modelo LoRA en {lora_path}[/red]")
        sys.exit(1)
    ...
    if args.base_model:
        ...
        # Save actualización temporal
        with open(adapter_config_path, "w") as f:
            json.dump(adapter_config, f, indent=2)

    try:
        # Run fusión
        merge_lora_adapter(args.lora_model, args.output_path, args.push_to_hub, args.hub_model_id)

        # Verify si se solicita
        if not args.no_verify:
            if verify_merged_model(output_path):
                ...
                console.print("\n [yellow]Advertencia: La verificación encontró problemas[/yellow]")
