...
from LoRa_train.lora_config import DataConfig, LoRAConfig, LoRAPresets, TrainingConfig
...
class TrainingMethod(Enum):
    """Methods de entrenamiento/compresión disponibles."""
...
class LayerTrainingConfig:
    """Configuration for entrenamiento para un tipo de capa."""
...
class AdvancedLoRAConfig(LoRAConfig):
    ...
    final_layers_count: int = 3  # Número de layers finales con config especial

    # Nuevos parameters globales
    enable_gradient_checkpointing: bool = True
    mixed_precision_training: bool = True
    compression_aware_training: bool = False

    def get_layer_config(
        self, layer_name: str, layer_position: int, total_layers: int
    ) -> LayerTrainingConfig:
        """Obtiene la configuración para una capa específica."""
        # Verify si es una capa final
    ...
    def _get_layer_type(self, layer_name: str) -> str:
        ...
        if any(x in name_lower for x in ["embed", "embedding"]):
            ...
            x in name_lower for x in ["attention", "attn", "q_proj", "k_proj", "v_proj", "o_proj"]
        ):
            return "attention"
        elif any(
            x in name_lower
            for x in ["mlp", "ffn", "fc1", "fc2", "gate_proj", "up_proj", "down_proj"]
        ):
            return "ffn"
...
- Configuración avanzada por layers
...
[dim]Presiona Ctrl+C para salir en cualquier momento[/dim]""",
    ...
    def _select_training_mode(self) -> str:
        ...
        modes = {
            ...
            "2": (
                "healing",
                "Healing de modelo comprimido",
                "Recuperar calidad después de compresión",
            ),
            "3": ("advanced", "Configuración avanzada", "Control total por tipo de capa"),
        }

        table = Table(show_header=False, box=None)
        for key, (_mode, title, desc) in modes.items():
    ...
    def _select_model(self) -> Tuple[str, Path]:
        ...
        if not models:
            raise ValueError("No models found en el directorio")
        ...
        for i, model in enumerate(models, 1):
            status = " Comprimido" if model["compressed"] else "Original"
            table.add_row(str(i), model["name"], f"{model['size_gb']:.1f} GB", status)
    ...
    def _select_and_configure_datasets(self) -> List[DatasetConfig]:
        ...
        console.print("\n[dim]Puedes seleccionar múltiples datasets (empty to finish)[/dim]")
    ...
    def _configure_standard(self) -> Tuple[LoRAConfig, TrainingConfig]:
        ...
        console.print("\n[bold] Configuration for LoRA:[/bold]\n")

        # Presets disponibles
        presets = {
            "1": ("conservative", "Conservative (r=8, epochs=1)"),
            "2": ("balanced", "Balanced (r=16, epochs=3)"),
            "3": ("aggressive", "Aggressive (r=32, epochs=5)"),
            "4": ("custom", "Custom"),
        }
    ...
    def _configure_healing(self) -> Tuple[LoRAConfig, TrainingConfig]:
        ...
        console.print("\n[bold]Configuration for Healing:[/bold]\n")
        console.print(
            "[dim]El healing usa and optimized configuration para recuperar calidad[/dim]\n"
        )
        ...
        if compression_level == "high":
            console.print(
                "[yellow]Compresión alta detectada - se recomienda configuración agresiva[/yellow]"
            )
            lora_config = LoRAConfig(
                r=64,
                lora_alpha=128,
        ...
        training_config.save_total_limit = 5  # Save más checkpoints

        return lora_config, training_config

    def _configure_advanced(self) -> Tuple[AdvancedLoRAConfig, TrainingConfig]:
        """Configuración avanzada por tipo de capa."""
        console.print("\n[bold] Configuración Avanzada por Layers:[/bold]\n")

        # Configuration base
        base_config = AdvancedLoRAConfig(r=16, lora_alpha=32, lora_dropout=0.1)

        # Configure cada tipo de capa
        layer_types = ["embedding", "attention", "ffn", "normalization", "output"]

        for layer_type in layer_types:
            console.print(f"\n[cyan]Configurando layers {layer_type.upper()}:[/cyan]")

            # Method de entrenamiento
            console.print("Methods disponibles:")
            console.print("  [1] LoRA estándar")
            console.print("  [2] LoRA + INT8")
            console.print("  [3] LoRA + Pruning")
            console.print("  [4] Sin entrenamiento")

            method_choice = Prompt.ask("Method", choices=["1", "2", "3", "4"], default="1")

            if method_choice == "4":
                continue

            # Create configuración para este tipo
            layer_config = LayerTrainingConfig(
                layer_type=layer_type,
                training_method=(
                    TrainingMethod.LORA_STANDARD
                    if method_choice == "1"
                    else (
                        TrainingMethod.LORA_INT8
                        if method_choice == "2"
                        else TrainingMethod.LORA_PRUNED
                    )
                ),
            )
            ...
            if method_choice != "4":
                layer_config.lora_rank = IntPrompt.ask(f"  Rank para {layer_type}", default=16)
                layer_config.learning_rate_multiplier = FloatPrompt.ask(
                    "  Multiplicador LR", default=1.0
                )

            base_config.layer_configs[layer_type] = layer_config

        # Configuration for layers finales
        if Confirm.ask("\n¿Configuración especial para layers finales?", default=True):
            num_final = IntPrompt.ask("Número de layers finales", default=3)
    ...
    def _configure_data(self) -> DataConfig:
        """Configura parameters de datos."""
        console.print("\n[bold]Configuration for datos:[/bold]\n")

        data_config = DataConfig()

        # Longitud máxima
        data_config.max_length = IntPrompt.ask("Longitud máxima de secuencia", default=512)
        ...
        # Split de evaluación
        data_config.eval_split_ratio = FloatPrompt.ask("Proporción para evaluación", default=0.1)
    ...
    def _review_configuration(self, lora_config, training_config, data_config) -> bool:
        ...
        # LoRA
        console.print("\n[cyan]LoRA:[/cyan]")
        console.print(f"  • Rank: {lora_config.r}")
    ...
    def _run_training(self, lora_config, training_config, data_config):
        ...
        # Create trainer
        trainer = LoRATrainer(
            model_name=self.model_name, model_path=self.model_path, output_dir=output_path
        )
    ...
    def _select_target_modules(self) -> List[str]:
        ...
        selection = Prompt.ask("\nSelection", default="1,2,3,4,5,6,7")
        ...
        if selection:
            ...
            selected = ["q_proj", "v_proj", "k_proj", "o_proj", "gate_proj", "up_proj", "down_proj"]
    ...
    def _estimate_resources(self, lora_config, training_config, num_samples):
        ...
        model_size_gb = sum(f.stat().st_size for f in self.model_path.rglob("*.safetensors")) / (
            1024**3
        )
        ...
        time_per_step = 0.5  # seconds, aproximado
    ...
    def _show_results(self, results: Dict[str, Any], output_path: Path):
        ...
        for file in saved_files[:5]:  # Show máximo 5
...
def main():
    ...
    parser.add_argument(
        "--quick", action="store_true", help="Modo rápido con configuración por defecto"
    )
