"""Profileado y análisis de layers individuales."""
...
class LayerProfiler:
    """Analiza y perfila layers de redes neuronales."""

    def __init__(self, architecture_config: Dict):
        self.arch_config = architecture_config

    def profile_layer(self, name: str, module: torch.nn.Module, model_stats: Dict) -> LayerProfile:
        """Profilea una capa individual."""
        # Information básica
        ...
        # Calculate potencial de compresión
    ...
    def _classify_layer(self, name: str, module: torch.nn.Module) -> str:
        ...
        # "normalization" únicamente aquellas layers que contenían la cadena
        ...
        # compresión, otro clasificador sí identificaba estas layers y se
        # generaban advertencias del tipo "No hay configuración para tipo
        # 'normalization'".  Esto hacía que las layers de normalización se
    ...
    def _add_architecture_specific_info(self, profile: LayerProfile, name: str, model_stats: Dict):
        ...
        if model_stats.get("is_moe") and profile.type in ["moe_expert", "ffn"]:
            ...
            # Initialize utilización uniforme por defecto
    ...
    def calculate_compression_potential(self, profile: LayerProfile) -> float:
        ...
        # Posición (layers finales más comprimibles)
        if profile.type in ["attention", "ffn"]:
            potential += profile.relative_position * 0.2

        # Size (layers grandes son mejores candidatas)
    ...
    def _recommend_compression_methods(self, profile: LayerProfile) -> List[Tuple[str, float]]:
        ...
        # MPO/Tucker para layers con alta correlación
        ...
        # Pruning para layers con alta sparsidad
        ...
        if profile.type not in ["embedding", "normalization"]:
            if profile.relative_position < 0.3:  # Layers tempranas
