...
class CompressionConfigManager:
    ...
    def load_from_report(self) -> bool:
        """Carga información de layers desde el reporte de análisis - OPTIMIZADO."""
        ...
        try:
            # Load reporte con optimizaciones
            ...
            # Process layers en paralelo si hay muchas
    ...
    def _process_layers_sequential(self, all_layers: Dict[str, Any]):
        """Procesa layers secuencialmente para modelos pequeños."""
    ...
    def _process_layers_parallel(self, all_layers: Dict[str, Any]):
        """Procesa layers en paralelo para modelos grandes."""
    ...
    def _show_layer_summary(self):
        """Muestra resumen de layers."""
        print("\nTypes de layers encontrados:")
        total_size = 0

        for layer_type, layers in sorted(self.layer_types.items()):
            type_size = sum(l["size_mb"] for l in layers)
            total_size += type_size
            print(f"  - {layer_type}: {len(layers)} layers, {type_size:.1f} MB")

        print(f"\n  Total: {total_size:.1f} MB en {self.total_layers} layers")

    def get_layer_types_list(self) -> List[str]:
        """Obtiene la lista de tipos de layers disponibles."""
        return list(self.layer_types.keys())

    def get_layers_by_type(self, layer_type: str) -> List[Dict[str, Any]]:
        """Obtiene todas las layers de un tipo específico."""
    ...
    def _print_missing_report_error(self):
        ...
        print("\nNot found reporte de análisis previo.")
    ...
    def set_compression_profile(self, profile_name: str):
        ...
        # Copiar config de layers del perfil seleccionado
        layer_configs = profile.get("layer_configs", {}).copy()

        # Types estándar que deben existir incluso si no aparecen en el
        # análisis del modelo. Esto garantiza que layers como las de
    ...
    def set_custom_layer_config(
        self, layer_type: str, methods: List[Dict[str, Any]], total_compression_ratio: float
    ):
    ...
    def add_position_modifiers(self):
        ...
        self.compression_config["position_modifiers"] = {
            "early_layers": {
                ...
                "description": "Layers iniciales (importantes para features)",
            },
            "middle_layers": {
                "range": [0.3, 0.7],
                "compression_multiplier": 1.0,
                "description": "Layers intermedias",
            },
            "late_layers": {
                "range": [0.7, 1.0],
                "compression_multiplier": 1.3,
                "description": "Layers finales (menos críticas)",
            },
        }

    def add_preserved_layers(self, layer_names: List[str]):
        """Marca layers específicas para preservar sin compresión."""
        ...
        self.compression_config["preserved_layers"] = sorted(preserved_set)
    ...
    def set_final_layers_config(self, num_layers: int, config: Dict[str, Any]):
        """Configura las últimas N layers con configuración especial."""
    ...
    def load_config(self, config_path: Path) -> Optional[Dict[str, Any]]:
        ...
        try:
            with open(config_path, encoding="utf-8") as f:
                config = json.load(f)
            logger.info(f"Config loaded desde: {config_path}")
            return config
        except Exception as e:
    ...
    def save_config(self, config: Dict[str, Any], config_path: Path) -> bool:
        ...
        try:
            ...
            logger.error(f"Error saving configuración: {e}")
            return False
    ...
    def validate_config(self, config: Dict[str, Any]) -> Tuple[bool, List[str]]:
        ...
        # Validar configuraciones de layers
                ...
                if not isinstance(methods, list):
                    ...
                    for i, method in enumerate(methods):
                        if not isinstance(method, dict):
                            errors.append(
                                f"Method {i} en capa '{layer_name}' debe ser un diccionario"
                            )
                            ...
                            if (
                                ...
                                errors.append(
                                    f"'strength' debe ser un número entre 0 y 1 en método {i} de capa '{layer_name}'"
                                )
        ...
        if is_valid:
            ...
            logger.warning(f"Configuración inválida: {len(errors)} errores")
            for error in errors:
