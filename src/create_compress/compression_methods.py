"""Methods de compresión optimizados para modelos de lenguaje."""
...
class CompressionMethod(ABC):
    ...
    @abstractmethod
    def compress(
        self, module: nn.Module, config: Dict[str, Any], device: torch.device
    ) -> nn.Module:
...
class QuantizationMethod(CompressionMethod):
    ...
    def _compress_pytorch2(
        self, module: nn.Module, strength: float, device: torch.device
    ) -> nn.Module:
        ...
        try:
            ...
            if isinstance(module, nn.Linear):
                ...
                # Apply cuantización dinámica
                quantized = torch.quantization.quantize_dynamic(
                    module, {nn.Linear}, dtype=torch.qint8 if self.bits == 8 else torch.quint4x2
                )
                return quantized
        except Exception as e:
    ...
    def _compress_legacy(
        ...
        with torch.no_grad():
            ...
            # Calculate escala y zero point según bits
            if self.bits == 2:
            ...
            # Apply strength (mezcla con original)
...
class PruningMethod(CompressionMethod):
    ...
    def _magnitude_pruning(self, module: nn.Module, strength: float) -> nn.Module:
        ...
        with torch.no_grad():
            ...
            # Calculate umbral de poda de forma eficiente
            ...
            # Create máscara de poda (usando el dtype original)
            mask = weight.abs() > threshold.to(original_dtype)

            # Apply máscara
            module.weight.data *= mask.to(original_dtype)

            # Save máscara para inferencia eficiente
    ...
    def _structured_pruning(self, module: nn.Module, strength: float) -> nn.Module:
        ...
        with torch.no_grad():
            ...
            # Calculate importancia por fila (neurona de salida)
            importance = weight.abs().mean(dim=1)

            # Select filas a mantener
            ...
            # Create nuevo módulo con menos neuronas
            new_module = nn.Linear(module.in_features, k, bias=module.bias is not None)
    ...
    def compress(
        ...
        with torch.no_grad():
            ...
            try:
                ...
                # Apply directamente al módulo original
...
class AttentionPruning(CompressionMethod):
    ...
    def compress(
        ...
        # Verify si es un módulo de atención
        ...
        for name, child in module.named_children():
            if any(
                proj in name
                for proj in ["q_proj", "k_proj", "v_proj", "o_proj", "c_attn", "c_proj"]
                ...
                with torch.no_grad():
                    ...
                    # Create máscara para las cabezas
                    ...
                    # Apply máscara directamente al peso
...
class MPOCompression(CompressionMethod):
    ...
    def compress(
        ...
        with torch.no_grad():
            ...
            # Configure rango MPO basado en strength
            ...
            try:
                ...
                weight_4d = weight.reshape(
                    factors_out[0], factors_out[1], factors_in[0], factors_in[1]
                )

                # Apply descomposición tensor_train
                factors = decomposition.tensor_train(weight_4d, rank=mpo_rank)
...
class ExpertPruning(CompressionMethod):
    ...
    def compress(
        ...
        if hasattr(module, "experts") and isinstance(module.experts, nn.ModuleList):
            ...
            # Calculate importancia de cada experto
            expert_importance = []
            for i, expert in enumerate(module.experts):
                # Calculate importancia como norma total de pesos
                importance = 0
                        ...
                        if i not in keep_indices:
                            if i < module.gate.weight.shape[0]:
                                ...
                                if module.gate.bias is not None and i < module.gate.bias.shape[0]:
                                    module.gate.bias.data[i] = -float(
                                        "inf"
                                    )  # Desactivar efectivamente

            logger.info(
                f"Poda de expertos: {num_experts - num_to_keep}/{num_experts} expertos eliminados"
            )
        ...
        # Buscar recursivamente en submódulos
            ...
            for name, child in module.named_children():
                if "expert" in name.lower() or "moe" in name.lower():
                    # Apply poda estructurada a expertos individuales
                    with torch.no_grad():
                        if hasattr(child, "weight"):
                            # Apply poda por magnitud al experto
                            weight = child.weight.data
...
class MixedPrecisionMethod(CompressionMethod):
    ...
    def compress(
        ...
        with torch.no_grad():
            ...
            # Calculate importancia de cada peso
            ...
            # Create máscaras para diferentes precisiones
            ...
            # Apply diferentes niveles de cuantización
            ...
            # Apply directamente al módulo
            module.weight.data = weight_mixed.to(original_dtype)

            logger.debug(
                f"Precisión mixta aplicada: {(mask_high.sum() / weight.numel() * 100):.1f}% alta, "
    ...
    def estimate_compression(self, module: nn.Module, config: Dict[str, Any]) -> float:
        ...
        # Calculate compresión promedio ponderada
        compression = (
            high_precision_ratio * 0  # Sin compresión
            + medium_precision_ratio * 0.5  # 50% compresión (INT8)
            + low_precision_ratio * 0.75
        )  # 75% compresión (INT4)
...
class BlockSparseMethod(CompressionMethod):
    ...
    def compress(
        ...
        with torch.no_grad():
            ...
            # Calculate importancia por bloque
            ...
            logger.debug(
                f"Sparsidad por bloques aplicada: {blocks_zeroed}/{total_blocks} bloques "
    ...
    def compress(
        ...
        try:
            with torch.no_grad():
                ...
                # Verify que las dimensiones sean compatibles con MPO
                ...
                # Verify que el rango sea válido
                ...
                try:
                    # Calculate factores que dividan las dimensiones
                    out_factors = self._factorize_dimension(out_features)
                    in_factors = self._factorize_dimension(in_features)

                    # Verify que el reshape sea válido
                    if (
                        out_factors[0] * out_factors[1] != out_features
                        or in_factors[0] * in_factors[1] != in_features
                    ):
                        logger.warning(
                            f"Dimensiones no factorizables para MPO: {out_features}x{in_features}"
                        )
                        # Fallback a SVD
                    ...
                    weight_4d = weight.view(
                        out_factors[0], out_factors[1], in_factors[0], in_factors[1]
                    )

                    # Apply descomposición MPO
                    decomposed = tl.decomposition.tensor_train(weight_4d, rank=rank)

                    # Create módulo comprimido
...
def get_compression_method(name: str) -> CompressionMethod:
    ...
    if name not in _COMPILED_METHODS_CACHE:
        if name not in COMPRESSION_METHODS:
            logger.warning(f"Method desconocido: {name}, usando 'none'")
            name = "none"

        # Create instancia y cachear
...
def apply_compression(
    module: nn.Module, method_config: Dict[str, Any], device: torch.device
) -> nn.Module:
...
def validate_compression_config(config: Dict[str, Any]) -> bool:
    ...
    if config["name"] not in COMPRESSION_METHODS:
        logger.error(f"Method desconocido: {config['name']}")
        return False
