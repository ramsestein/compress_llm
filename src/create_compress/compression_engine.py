...
def _safe_quantiles(
    tensor: torch.Tensor, qs: List[float], sample_size: int = 100000
) -> torch.Tensor:
...
class CompressionEngine:
    """Motor ligero para aplicar técnicas de compresión a layers individuales.
    ...
    def apply_method(
        ...
        ----------
        module:
            Layer del modelo a modificar.
        method_name:
            Name del método (por ejemplo ``int8_quantization``).
        strength:
            Strength del método entre 0 y 1.
        layer_config:
            ...
            for name, module in model.named_modules():
                if name in config.get("layer_configs", {}):
                    ...
                    if result.success:
                        ...
                        logger.info(f"Layer {name} comprimida: {result.compression_ratio:.2%}")
                    else:
                        logger.warning(f"Fallo comprimiendo capa {name}: {result.error}")
    ...
    def _apply_int8_quantization(
        self, module: nn.Module, strength: float, config: Dict
    ) -> nn.Module:
        ...
        quantized = QuantizedLinear(
            module.in_features, module.out_features, bias=module.bias is not None, bits=8
        )
    ...
    def _apply_int4_quantization(
        ...
        quantized = QuantizedLinear(
            module.in_features, module.out_features, bias=module.bias is not None, bits=4
        )
    ...
    def _apply_int2_quantization(
        ...
        ternary.weight.data = torch.sign(module.weight.data) * (
            module.weight.data.abs() > threshold
        )

        # Create el buffer scale de forma segura
    ...
    def _apply_pruning(self, module: nn.Module, strength: float, config: Dict) -> nn.Module:
        ...
        # Create máscara de poda usando cuantiles robustos
    ...
    def _apply_tucker_decomposition(
        ...
        # Calculate rangos basados en strength
        rank_in = max(1, int(module.in_features * (1 - strength * 0.7)))
        rank_out = max(1, int(module.out_features * (1 - strength * 0.7)))

        tucker = TuckerLinear(
            module.in_features, module.out_features, rank_in, rank_out, bias=module.bias is not None
        )
    ...
    def _apply_mpo_decomposition(
        ...
        mpo = MPOLinear(
            module.in_features, module.out_features, bond_dims, bias=module.bias is not None
        )

        # Initialize con descomposición TT
        mpo.initialize_from_linear(module)

        return mpo

    def _apply_svd_decomposition(
        self, module: nn.Module, strength: float, config: Dict
    ) -> nn.Module:
        ...
        # Calculate rango basado en strength
        max_rank = min(module.in_features, module.out_features)
        rank = max(1, int(max_rank * (1 - strength)))

        svd = SVDLinear(module.in_features, module.out_features, rank, bias=module.bias is not None)
    ...
    def _quantize_tensor(
        self, tensor: torch.Tensor, scale: float, zero_point: int, bits: int
    ) -> torch.Tensor:
        """Cuantiza un tensor."""
        qmin = 0
        qmax = 2**bits - 1
...
# Classes auxiliares para diferentes tipos de compresión


class QuantizedLinear(nn.Module):
    """Layer lineal cuantizada."""
...
class TuckerLinear(nn.Module):
    """Layer lineal con descomposición Tucker."""
...
class SVDLinear(nn.Module):
    """Layer lineal con descomposición SVD."""
...
class LoRALinear(nn.Module):
    """Layer lineal con adaptadores LoRA."""
...
class MixedPrecisionLinear(nn.Module):
    """Layer lineal con precisión mixta."""
    ...
    def __init__(self, in_features, out_features, bias=True):
        ...
        self.register_buffer(
            "high_precision_mask", torch.zeros(out_features, in_features, dtype=torch.bool)
        )
        self.register_buffer(
            "medium_precision_mask", torch.zeros(out_features, in_features, dtype=torch.bool)
        )

        # Pesos en diferentes precisiones
        self.weight_high = nn.Parameter(torch.zeros(out_features, in_features))
        self.register_buffer(
            "weight_medium", torch.zeros(out_features, in_features, dtype=torch.float16)
        )
    ...
    def set_weight_importance(self, weight):
        ...
        self.high_precision_mask = importance > high_threshold.to(
            importance.device, importance.dtype
        )
        self.medium_precision_mask = (
            importance > medium_threshold.to(importance.device, importance.dtype)
        ) & ~self.high_precision_mask
...
class BlockSparseLinear(nn.Module):
    """Layer lineal con sparsidad por bloques."""
...
class MPOLinear(nn.Module):
    """Layer lineal con Matrix Product Operators."""
    ...
    def __init__(self, in_features, out_features, bond_dims, bias=True):
        ...
        for i in range(n_cores):
            core = nn.Parameter(torch.randn(dims[i], out_dims[i], in_dims[i], dims[i + 1]))
