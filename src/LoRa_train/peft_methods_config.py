...
PEFTPresets = {
    "efficient": {
        ...
        "bitfit": BitFitConfig(method=PEFTMethod.BITFIT, learning_rate=5e-5, num_train_epochs=1),
        "ia3": IA3Config(method=PEFTMethod.IA3, learning_rate=1e-3, num_train_epochs=1),
    },
    "balanced": {
        "lora": LoRAConfig(
            method=PEFTMethod.LORA,
        ...
        "dora": DoRAConfig(
            method=PEFTMethod.DORA, r=16, lora_alpha=32, learning_rate=2e-4, num_train_epochs=3
        ),
        "adapter": AdapterConfig(
            method=PEFTMethod.ADAPTER, adapter_size=64, learning_rate=1e-4, num_train_epochs=3
        ),
    },
    "quality": {
        "lora": LoRAConfig(
    ...
    "memory_efficient": {
        ...
        "galore": GaLoreConfig(
            method=PEFTMethod.GALORE, rank=64, scale=0.25, learning_rate=5e-5, num_train_epochs=1
        ),
...
def get_config_by_name(preset_name: str, method_name: str) -> BasePEFTConfig:
    ...
    if method_name not in PEFTPresets[preset_name]:
        raise ValueError(f"Method '{method_name}' not found en preset '{preset_name}'")
