...
class TestOptimizedDatasetManager(unittest.TestCase):
    ...
    def _create_test_datasets(self):
        ...
        pd.DataFrame(supervised_data).to_csv(
            self.datasets_dir / "supervised_dataset.csv", index=False
        )
        ...
        pd.DataFrame(conversation_data).to_csv(
            self.datasets_dir / "conversation_dataset.csv", index=False
        )
    ...
    def test_scan_datasets(self):
        ...
        # Verify que se encontraron datasets
        self.assertIsInstance(datasets, list)
        self.assertGreater(len(datasets), 0)

        # Verify estructura de cada dataset
    ...
    def test_analyze_dataset_supervised(self):
        ...
        # Verify que se detectaron las columnas correctas
    ...
    def test_load_dataset_supervised(self):
        ...
        # Verify que se cargó correctamente
    ...
    def test_error_handling_empty_dataset(self):
        ...
        # Create un CSV vacío pero con header
        ...
        # Verify que se detectó como vacío pero sin error (solo header)
    ...
    def test_cache_functionality(self):
        ...
        # Verify que se creó el archivo de cache
        self.assertTrue(cache_file.exists())

        # Verify que el cache contiene datos válidos
        with open(cache_file) as f:
            cache_data = json.load(f)
        self.assertIsInstance(cache_data, dict)
        self.assertIn("datasets", cache_data)

        # Verify que el cache tiene el mismo número de datasets
    ...
    def test_column_detection(self):
        ...
        # Create dataset con columnas específicas
        test_data = {
    ...
    def test_parallel_processing(self):
        ...
        # Verify que se procesaron correctamente
        # Deberían ser al menos 3 (los datasets creados en setUp)
        self.assertGreaterEqual(len(datasets), 3)

        # Verify que todos los datasets tienen la estructura correcta
        for dataset in datasets:
            self.assertIn("file_path", dataset)
            self.assertIn("name", dataset)
            self.assertIn("format", dataset)
    ...
    def test_hash_generation(self):
        ...
        # Verify que se generan hashes diferentes
        hash1 = config1.hash
        hash2 = config2.hash
