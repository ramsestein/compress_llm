...
TENSORLY_AVAILABLE = importlib.util.find_spec("tensorly") is not None
...
class TestCompressionSystemComprehensive(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # Create directorio temporal para el modelo
        self.model_path = self.test_dir / "test_model"
        self.model_path.mkdir(parents=True, exist_ok=True)

        # Create archivo de configuración mock
        ...
        # Initialize componentes del sistema
    ...
    def test_compression_engine_initialization(self):
        """Test of inicialización del motor de compresión."""
        # Verify inicialización básica
    ...
    def test_quantization_methods(self):
        """Test of métodos de cuantización."""
        # Test INT8
        result = self.engine.apply_method(
            self.linear_layer, "int8_quantization", 0.5, {"params": {}}
        )
        self.assertIsNotNone(result)

        # Test INT4
        result = self.engine.apply_method(
            self.linear_layer, "int4_quantization", 0.5, {"params": {}}
        )
        self.assertIsNotNone(result)

        # Test INT2
        result = self.engine.apply_method(
            self.linear_layer, "int2_quantization", 0.5, {"params": {}}
        )
        self.assertIsNotNone(result)

    def test_pruning_methods(self):
        """Test of métodos de poda."""
        # Test magnitude pruning
        result = self.engine.apply_method(
            self.linear_layer, "magnitude_pruning", 0.3, {"params": {}}
        )
        self.assertIsNotNone(result)

        # Test structured pruning
        result = self.engine.apply_method(
            self.linear_layer, "structured_pruning", 0.3, {"params": {}}
        )
        self.assertIsNotNone(result)

        # Test attention pruning
        result = self.engine.apply_method(
            self.attention_layer, "attention_pruning", 0.3, {"params": {}}
        )
        self.assertIsNotNone(result)

    def test_decomposition_methods(self):
        """Test of métodos de descomposición."""
        # Test SVD
        result = self.engine.apply_method(
            self.linear_layer, "low_rank_approximation", 0.3, {"params": {}}
        )
        self.assertIsNotNone(result)

        # Test Tucker
        result = self.engine.apply_method(
            self.linear_layer, "tucker_decomposition", 0.3, {"params": {}}
        )
        self.assertIsNotNone(result)

        # Test MPO (requires tensorly)
        if TENSORLY_AVAILABLE:
            result = self.engine.apply_method(self.linear_layer, "mpo", 0.3, {"params": {}})
            self.assertIsNotNone(result)

    def test_compression_layer_functionality(self):
        """Test of funcionalidad de compresión de layers."""
        # Test con método válido
        compressed_layer, result = self.engine.compress_layer(
            self.linear_layer, {"name": "int8_quantization", "strength": 0.5}
        )
        ...
        compressed_layer, result = self.engine.compress_layer(
            self.linear_layer, {"name": "invalid_method", "strength": 0.5}
        )
        self.assertIsNotNone(compressed_layer)
    ...
    def test_quantization_parameters_calculation(self):
        """Test of cálculo de parámetros de cuantización."""
    ...
    def test_tensor_quantization(self):
        """Test of cuantización de tensores."""
    ...
    def test_quantization_method_class(self):
        """Test of la clase QuantizationMethod."""
        method = QuantizationMethod(bits=8)

        # Test of estimación de compresión
        estimated_compression = method.estimate_compression(self.linear_layer, {"strength": 0.5})
        ...
        # Test of compresión
        compressed = method.compress(self.linear_layer, {"strength": 0.5}, self.device)
        self.assertIsNotNone(compressed)

    def test_pruning_method_class(self):
        """Test of la clase PruningMethod."""
        method = PruningMethod()

        # Test of estimación de compresión
        estimated_compression = method.estimate_compression(self.linear_layer, {"strength": 0.3})
        self.assertIsInstance(estimated_compression, float)
        self.assertGreaterEqual(estimated_compression, 0)
        self.assertLessEqual(estimated_compression, 1)

        # Test of compresión
        compressed = method.compress(self.linear_layer, {"strength": 0.3}, self.device)
        self.assertIsNotNone(compressed)

    def test_low_rank_approximation_class(self):
        """Test of la clase LowRankApproximation."""
    ...
    @unittest.skipUnless(TENSORLY_AVAILABLE, "tensorly not installed")
    def test_mpo_decomposition_class(self):
        """Test of la clase MPODecomposition."""
        ...
        # Test of factorización de dimensiones
    ...
    def test_conservative_profile(self):
        ...
        # Verify que los métodos son conservadores
    ...
    def test_balanced_profile(self):
        ...
        # Verify que los métodos están balanceados
        attention_config = layer_configs["attention"]
    ...
    def test_aggressive_profile(self):
        ...
        # Verify que los métodos son agresivos
        attention_config = layer_configs["attention"]
        self.assertGreater(attention_config["total_compression_ratio"], 0.5)

    def test_layer_configs_structure(self):
        """Test of estructura de configuraciones de layers."""
        for _profile_name, profile in COMPRESSION_PROFILES.items():
            layer_configs = profile["layer_configs"]

            for _layer_type, config in layer_configs.items():
                # Verify estructura básica
                self.assertIn("methods", config)
                self.assertIn("total_compression_ratio", config)

                # Verify que methods es una lista
    ...
    def test_config_validation(self):
        """Test of validación de configuraciones."""
        ...
        invalid_config = {
            "metadata": {},
            "global_settings": {"target_compression": 2.0},  # Valor inválido
        }
    ...
    def test_config_save_load(self):
        """Test of guardado y carga de configuraciones."""
        config = {
            "metadata": {"model_name": "test_model", "version": "1.0"},
            "global_settings": {
                "name": "Balanced",
    ...
    def test_interactive_config_builder(self):
        """Test ofl constructor de configuración interactiva."""
    ...
    def test_layer_config_customization(self):
        """Test of personalización de configuraciones de layers."""
    ...
    def test_end_to_end_compression_workflow(self):
        """Test of flujo completo de compresión."""
        # Create configuración
        config = {
            "metadata": {"model_name": "test_model", "version": "1.0"},
            "global_settings": {
                "name": "Balanced",
    ...
    def test_compression_result_structure(self):
        """Test of estructura del resultado de compresión."""
        # Create resultado de compresión
    ...
    def test_memory_efficiency(self):
        """Test of eficiencia de memoria."""
        # Create capa grande
        large_layer = nn.Linear(1000, 1000)

        # Apply compresión
        compressed_layer, result = self.engine.compress_layer(
            large_layer, {"name": "int8_quantization", "strength": 0.5}
        )

        # Verify que la compresión fue efectiva
        self.assertTrue(result.success)
        self.assertGreater(result.compression_ratio, 0)
    ...
    def test_error_handling(self):
        ...
        compressed_layer, result = self.engine.compress_layer(
            self.linear_layer, {"name": "nonexistent_method", "strength": 0.5}
        )
    ...
    def test_device_compatibility(self):
        ...
        if torch.cuda.is_available():
            ...
            result = gpu_engine.apply_method(
                self.linear_layer.cuda(), "int8_quantization", 0.5, {"params": {}}
            )
            self.assertIsNotNone(result)
    ...
    def test_numerical_stability(self):
        ...
        result = self.engine.apply_method(extreme_layer, "int8_quantization", 0.5, {"params": {}})
        self.assertIsNotNone(result)
        ...
        result = self.engine.apply_method(small_layer, "int8_quantization", 0.5, {"params": {}})
        self.assertIsNotNone(result)

    def test_reproducibility(self):
        """Test of reproducibilidad."""
        # Apply la misma compresión dos veces
