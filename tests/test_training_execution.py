...
class TestTrainingExecution(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # Create directorio del modelo
        self.model_path.mkdir(exist_ok=True)

        # Create archivos de configuración simulados
        config = {
            "model_type": "gpt2",
            "vocab_size": 50257,
            "n_positions": 1024,
    ...
    def test_lora_training_execution(self, mock_get_peft_model, mock_tokenizer, mock_model):
        ...
        # Run entrenamiento
        with patch.object(trainer, "_setup_training"), patch.object(
            trainer, "_execute_training"
        ) as mock_execute, patch.object(trainer, "_save_model"):
    ...
    def test_ia3_training_execution(self, mock_get_peft_model, mock_tokenizer, mock_model):
        ...
        # Create trainer
        config = IA3Config(method=PEFTMethod.IA3, target_modules=["attn.c_attn", "attn.c_proj"])
        trainer = PEFTUniversalTrainer("test_model", self.model_path, self.output_dir, config)
    ...
    def test_bitfit_training_execution(self, mock_get_peft_model, mock_tokenizer, mock_model):
        ...
        # Create trainer
        config = BitFitConfig(
            method=PEFTMethod.BITFIT, target_modules=["attn.c_attn", "attn.c_proj"]
        )
        trainer = PEFTUniversalTrainer("test_model", self.model_path, self.output_dir, config)
    ...
    def test_error_handling_invalid_method(self):
        ...
        # Create configuración con método inválido
        config = Mock()
        config.method = "invalid_method"

        trainer = PEFTUniversalTrainer("test_model", self.model_path, self.output_dir, config)

        # Verify que se lanza error al intentar entrenar
    ...
    def test_error_handling_model_loading_failure(self):
        ...
        # Create trainer con modelo inexistente
        config = LoRAConfig(
            method=PEFTMethod.LORA,
            r=8,
            lora_alpha=16,
        ...
        trainer = PEFTUniversalTrainer(
            "test_model", Path("/path/inexistente"), self.output_dir, config
        )
    ...
    def test_dataset_preparation_with_different_formats(self, mock_tokenizer, mock_model):
        ...
        # Verify que se crearon los datasets
