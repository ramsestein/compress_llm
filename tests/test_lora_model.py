...
class TestLoRAModel(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # Create directorio temporal
        self.temp_dir = tempfile.mkdtemp()
        self.dummy_model_dir = Path(self.temp_dir)

        # Create archivo de configuración del adaptador
    ...
    def test_batch_test_returns_expected_keys(self):
        ...
        with patch.object(LoRAModelTester, "load_model", return_value=None), patch.object(
            LoRAModelTester,
            "generate",
            ...
            for res, prompt in zip(results, prompts):
                ...
                self.assertTrue({"prompt", "response", "success"}.issubset(res.keys()))
