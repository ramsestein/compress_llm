...
class OptimizedDatasetManager:
    ...
    def scan_datasets(self, use_cache: bool = True) -> List[Dict[str, Any]]:
        ...
        if use_cache and cache_file.exists():
            try:
                ...
                # Verify que los archivos aún existen
                valid_datasets = []
                ...
                if len(valid_datasets) == len(cached_data["datasets"]):
                    logger.info(f"Loaded {len(valid_datasets)} datasets desde cache")
    ...
    def _detect_format(self, file_path: Path) -> str:
        ...
        if suffix == ".csv":
            ...
            # Verify si es JSONL (una línea por objeto)
    ...
    def _analyze_dataset(self, file_path: Path) -> Dict[str, Any]:
        ...
        # Analysis específico por formato
        if format_type == "csv":
    ...
    def _analyze_csv(self, file_path: Path, info: Dict[str, Any]):
        ...
        try:
            ...
            if sample.shape[1] == 1 and "," not in str(sample.iloc[0, 0]):
                ...
                for delimiter in ["\t", "|", ";"]:
                    sample = pd.read_csv(
                        file_path, nrows=1000, delimiter=delimiter, on_bad_lines="skip"
                    )
                    if sample.shape[1] > 1:
            ...
            info["estimated_rows"] = (
                int(file_path.stat().st_size / bytes_per_row) if bytes_per_row > 0 else 0
            )
    ...
    def configure_dataset_interactive(
        ...
        try:
            ...
            # Configuration adicional
            while True:
                try:
                    max_length_input = input(
                        "Longitud máxima de secuencia (default: 512): "
                    ).strip()
            ...
            while True:
                try:
                    eval_ratio_input = input(
                        "Proporción para evaluación 0.0-1.0 (default: 0.1): "
                    ).strip()
            ...
            print(f"Type: {self._get_dataset_type_name(dataset_type)}")
    ...
    def _configure_supervised_dataset(self, dataset_info: Dict[str, Any]) -> DatasetConfig:
        ...
        input_col = input(
            "Columna de entrada (instrucción/pregunta) [default: instruction]: "
        ).strip()
        if not input_col:
    ...
    def _configure_conversation_dataset(self, dataset_info: Dict[str, Any]) -> DatasetConfig:
        ...
        while True:
            format_choice = input("Format de conversación (1-3) [default: 1]: ").strip()
    ...
    def load_dataset(
        self, config: DatasetConfig, split: str = "train", streaming: bool = False
        ...
        if config.format == "csv":
            ...
            raise ValueError(f"Format no soportado: {config.format}")

        # Apply splits si es necesario
        if not streaming and split != "full":
            data = self._apply_splits(data, config, split)

        # Save en cache si no es streaming
    ...
    def _load_jsonl(self, config: DatasetConfig, streaming: bool) -> Union[pd.DataFrame, Iterator]:
        ...
        if streaming:
            ...
            def jsonl_iterator():
                with open(config.file_path, encoding=config.encoding) as f:
                    batch = []
    ...
    def _load_parquet(
        self, config: DatasetConfig, streaming: bool
    ) -> Union[pd.DataFrame, Iterator]:
    ...
    def _apply_splits(self, data: pd.DataFrame, config: DatasetConfig, split: str) -> pd.DataFrame:
    ...
    def prepare_for_training(
        self, config: DatasetConfig, tokenizer: Any, max_length: int = 512
    ) -> Dict[str, Any]:
