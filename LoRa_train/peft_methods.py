...
class MoLoRALinear(BasePEFTModule):
    """Layer Linear con Mixture of LoRAs."""
    ...
    def __init__(self, in_features: int, out_features: int, config: "MoLoRAConfig"):
        ...
        # Create múltiples expertos LoRA
        ...
        for i in range(2):  # Top-2
            ...
            for expert_idx in range(self.config.num_experts):
                ...
                if mask.any():
                    ...
                    # Apply LoRA del experto
                    lora_out = self.lora_Bs[expert_idx](self.lora_As[expert_idx](expert_input))
...
class GaLoreLinear(nn.Linear):
    ...
    def __init__(
        self, in_features: int, out_features: int, config: "GaLoreConfig", bias: bool = True
    ):
        super().__init__(in_features, out_features, bias)
        self.config = config

        # Proyector para gradientes
        self.projector = GaLoreProjector((out_features, in_features), config.rank, config.proj_type)
...
class DoRALinear(BasePEFTModule):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        lora_direction = self.dropout(self.lora_B @ self.lora_A) * (
    ...
    def merge_weights(self):
        with torch.no_grad():
            ...
            direction = self.weight + self.lora_B @ self.lora_A * (
                self.config.lora_alpha / self.config.r
            )
            if self.config.normalize_direction:
                direction = F.normalize(direction, dim=1)
...
class BitFitModel(nn.Module):
    ...
    def __init__(self, model: nn.Module, config: "BitFitConfig"):
        ...
        for name, param in self.model.named_parameters():
            if self._should_train(name):
                ...
                logger.info(f"BitFit: Training {name}")
    ...
    def _should_train(self, param_name: str) -> bool:
        ...
        return bool(self.config.train_embeddings and "embed" in param_name)
...
class IA3Linear(BasePEFTModule):
    ...
    def __init__(self, base_layer: nn.Linear, config: "IA3Config", is_feedforward: bool = False):
        ...
        if is_feedforward:
            ...
            self.ia3_weights = nn.Parameter(torch.full((base_layer.out_features,), init_value))
        else:
            # Para attention K,V, escalar entrada
            self.ia3_weights = nn.Parameter(torch.full((base_layer.in_features,), init_value))
...
class QuantizedLoRALinear(BasePEFTModule):
    ...
    def _quantize_weight(self, weight: torch.Tensor) -> torch.Tensor:
        ...
        if self.bits == 4:
            ...
            return torch.quantize_per_tensor(
                weight, scale=1.0, zero_point=0, dtype=torch.qint8
            ).dequantize()
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Apply LoRA cuantizado
        lora_out = F.linear(self.dropout(F.linear(x, quantized_A)), quantized_B)
...
class PrunedLoRALinear(BasePEFTModule):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Apply máscaras de poda
        masked_A = self.lora_A * self.A_mask
        masked_B = self.lora_B * self.B_mask

        # Apply LoRA podado
        lora_out = F.linear(self.dropout(F.linear(x, masked_A)), masked_B)
...
class PromptEncoder(nn.Module):
    ...
    def __init__(self, config: "PromptTuningConfig", embedding_dim: int):
        ...
        if config.prompt_tuning_init == "random":
            ...
            # Initialize desde texto (requiere tokenizer)
            self.prompt_embeddings = nn.Parameter(
...
class S4Layer(nn.Module):
    """Layer S4 simplificada para el adaptador."""
...
class AdapterLayer(nn.Module):
    """Layer de adaptador individual para Houlsby."""
...
class AdapterLinear(BasePEFTModule):
    """Layer Linear con Adapter (bottleneck)."""
...
def _replace_layers(model: nn.Module, config: "BasePEFTConfig") -> None:
    ...
    for name, module in model.named_modules():
        ...
        if config.method == PEFTMethod.LORA and isinstance(module, nn.Linear):
            ...
            new_module = GaLoreLinear(
                module.in_features, module.out_features, config, bias=module.bias is not None
            )
            new_module.weight.data = module.weight.data.clone()
